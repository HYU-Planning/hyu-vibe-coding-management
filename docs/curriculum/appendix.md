...
## 웹서버 · 애플리케이션 서버 · DB서버 기본 구조

```mermaid
flowchart TD
    A[클라이언트<br/>브라우저 · 요청을 보내는 화면] -- 요청 --> B[웹서버<br/>Web Server · 정적 파일과 요청을 처리]
    B -- 요청 --> C[애플리케이션 서버<br/>App/API Server · 비즈니스 로직 실행]
    C -- 요청 --> D[DB 서버<br/>Database Server · 데이터 저장 · 조회]
```
